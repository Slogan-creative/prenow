...
export default async function Settings({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<{tab?:string;saved?:string;error?:string}>}){
 ...
 const [tenant,settings,branding,services,hours,members]=await Promise.all([
 ...
 <SettingsManager slug={slug} initialTab={selected} tenant={tenant.data||{}} settings={settings.data||{}} branding={branding.data||{}} services={services.data||[]} hours={hours.data||[]} operators={(members.data||[]) as Member[]}/></>;
}
