...
export default async function Applications() {
  ...
  return <><h1 className="page-title">Applicazioni</h1>
    ...
    <tbody>{tenants.map(t => <tr key={t.id}><td><a href={`/admin/applicazioni/${t.id}`}>{t.nome}</a></td><td>{t.piano_nome || '—'}</td><td>{t.stato}</td><td>{t.numero_operatori}</td><td>{t.slug}.prenow.it</td></tr>)}</tbody>
