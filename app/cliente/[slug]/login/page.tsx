...
export default function Login(){
 const {slug}=useParams<{slug:string}>();const router=useRouter();const [signup,setSignup]=useState(false);const [busy,setBusy]=useState(false);const [message,setMessage]=useState('');
 async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);setMessage('');const f=new FormData(e.currentTarget);const db=createBrowserSupabaseClient();const email=String(f.get('email'));const password=String(f.get('password'));
 try{const r=signup?await db.auth.signUp({email,password,options:{emailRedirectTo:`${window.location.origin}/cliente/${slug}/login`}}):await db.auth.signInWithPassword({email,password});
 ...
 router.push(`/cliente/${slug}`);router.refresh();}catch{setMessage('Collegamento non disponibile. Riprova.');}finally{setBusy(false);}}
