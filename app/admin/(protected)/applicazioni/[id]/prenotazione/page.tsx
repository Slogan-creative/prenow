...
export default async function ManualBooking({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|undefined>>}){
 await requirePlatformAdmin();const {id}=await params;try{entityId(id);}catch{notFound();}
 const q=await searchParams,db=await createServerClient(),path=`/admin/applicazioni/${id}/prenotazione`;
 ...
 redirect(`/admin/agenda?tenant=${id}&date=${form.get('date')}`);
 }
 const fmt=new Intl.DateTimeFormat('it-IT',{timeZone:'Europe/Rome',hour:'2-digit',minute:'2-digit'});
 return <><Link href={`/admin/applicazioni/${id}`}>← Configurazione</Link><h1>{tenant.data.nome}: prenotazione manuale</h1><p>Usa un cliente già presente nel salone. Sono rispettati orari, assegnazioni, chiusure e disponibilità.</p>
