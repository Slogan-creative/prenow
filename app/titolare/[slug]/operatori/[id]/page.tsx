...
const normalizeRange=(range:TimeRange|string[]):[string,string]=>Array.isArray(range)?[range[0]||'',range[1]||'']:[range.da||'',range.a||''];
...
export default async function OperatorDetail({params,searchParams}:{params:Promise<{slug:string;id:string}>;searchParams:Promise<{tab?:string;saved?:string;error?:string}>}){
 ...
 {tab==='orari'&&<section className="owner-hours-list">{DAYS.map(([name,weekday])=>{const day=hourMap.get(weekday)||{weekday,chiuso:true,fasce:[]};const ranges=(day.fasce?.length?day.fasce:[{da:'09:00',a:'13:00'}]).map(normalizeRange);return <form action={saveOperatorHours} className="owner-panel owner-day-editor" key={weekday}><input type="hidden" name="slug" value={slug}/><input type="hidden" name="operator_id" value={id}/><input type="hidden" name="weekday" value={weekday}/><header><b>{name}</b><label className="check"><input name="chiuso" type="checkbox" defaultChecked={day.chiuso}/> Chiuso</label></header>{ranges.map((r,i)=><div className="owner-range-row" key={i}><input type="time" name="start" defaultValue={r[0]}/><span>–</span><input type="time" name="end" defaultValue={r[1]}/></div>)}<button className="owner-secondary">Salva {name}</button></form>})}</section>}
