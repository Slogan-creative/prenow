...
begin ctx:=public.prenow_owner_context(p_slug);if ctx->>'role' not in ('tenant_admin','staff') then raise exception 'Accesso non autorizzato';end if;t:=(ctx->>'tenant_id')::uuid;
if p_start is null or p_end is null or p_end<p_start or length(coalesce(p_reason,''))>500 then raise exception 'Dati non validi';end if;perform 1 from public.operators where id=p_operator and tenant_id=t;if not found then raise exception 'Operatore non trovato';end if;
insert into public.operator_absences(tenant_id,operator_id,data_inizio,data_fine,motivo) values(t,p_operator,p_start,p_end,nullif(trim(coalesce(p_reason,'')),'')) returning id into result;return result;end $function$;

CREATE OR REPLACE FUNCTION public.prenow_owner_context(p_slug text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
 ...
 if coalesce(public.is_platform_admin(),false) then r:='tenant_admin';
 else select ruolo,operator_id into r,op from public.tenant_users where tenant_id=t and user_id=auth.uid() and attivo=true;end if;
...
CREATE OR REPLACE FUNCTION public.prenow_owner_delete_absence(p_slug text, p_operator uuid, p_absence uuid)
 ...
 SET search_path TO ''
AS $function$
declare ctx jsonb;t uuid;begin ctx:=public.prenow_owner_context(p_slug);if ctx->>'role' not in ('tenant_admin','staff') then raise exception 'Accesso non autorizzato';end if;t:=(ctx->>'tenant_id')::uuid;
delete from public.operator_absences where id=p_absence and operator_id=p_operator and tenant_id=t;if not found then raise exception 'Assenza non trovata';end if;end $function$;
...
begin
 ...
 if length(trim(coalesce(p_name,''))) not between 1 and 120 or (normalized_email is not null and normalized_email !~ '^[^[:space:]@]+@[^[:space:]@]+[.][^[:space:]@]+$') or p_role not in ('staff','operator') or p_active is null then raise exception 'Dati non validi';end if;
 ...
 if oid is null then insert into public.operators(tenant_id,nome,cognome,email,telefono,attivo) values(t,trim(p_name),nullif(trim(coalesce(p_surname,'')),''),normalized_email,nullif(trim(coalesce(p_phone,'')),''),p_active) returning id into oid;
 else update public.operators set nome=trim(p_name),cognome=nullif(trim(coalesce(p_surname,'')),''),email=normalized_email,telefono=nullif(trim(coalesce(p_phone,'')),''),attivo=p_active,updated_at=now() where id=oid and tenant_id=t;if not found then raise exception 'Operatore non trovato';end if;end if;
 if p_membership_id is null then insert into public.tenant_users(tenant_id,user_id,ruolo,nome,cognome,email,telefono,operator_id,attivo) values(t,uid,p_role,trim(p_name),nullif(trim(coalesce(p_surname,'')),''),normalized_email,nullif(trim(coalesce(p_phone,'')),''),oid,p_active) returning id into mid;
 else if exists(select 1 from public.tenant_users where id=p_membership_id and tenant_id=t and user_id=auth.uid()) then raise exception 'Non puoi modificare il tuo accesso';end if;
 update public.tenant_users set user_id=coalesce(user_id,uid),ruolo=p_role,nome=trim(p_name),cognome=nullif(trim(coalesce(p_surname,'')),''),email=normalized_email,telefono=nullif(trim(coalesce(p_phone,'')),''),operator_id=oid,attivo=p_active where id=p_membership_id and tenant_id=t returning id into mid;if mid is null then raise exception 'Collaboratore non trovato';end if;end if;
...
AS $function$
declare ctx jsonb;t uuid;begin ctx:=public.prenow_owner_context(p_slug);if ctx->>'role' not in ('tenant_admin','staff') then raise exception 'Accesso non autorizzato';end if;t:=(ctx->>'tenant_id')::uuid;
update public.operators set attivo=false,updated_at=now() where id=p_operator and tenant_id=t;if not found then raise exception 'Operatore non trovato';end if;update public.tenant_users set attivo=false where tenant_id=t and operator_id=p_operator and user_id is distinct from auth.uid();end $function$;
...
if not found then insert into public.business_hours(tenant_id,operator_id,weekday,chiuso,fasce) values(t,p_operator,p_weekday,p_closed,case when p_closed then '[]'::jsonb else p_ranges end);end if;end $function$;

CREATE OR REPLACE FUNCTION public.prenow_owner_staff(p_slug text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
begin ctx:=public.prenow_owner_context(p_slug);t:=(ctx->>'tenant_id')::uuid;role_name:=ctx->>'role';own_operator:=nullif(ctx->>'operator_id','')::uuid;select nome into title from public.tenants where id=t;
return jsonb_build_object('nome',title,'role',role_name,'appointments',coalesce((select jsonb_agg(jsonb_build_object('id',a.id,'start_at',a.start_at,'end_at',a.end_at,'stato',a.stato,'operator_id',a.operator_id,'customer',c.nome||' '||coalesce(c.cognome,''),'operator',o.nome,'service',s.nome) order by a.start_at) from public.appointments a join public.customers c on c.id=a.customer_id and c.tenant_id=t join public.operators o on o.id=a.operator_id and o.tenant_id=t join public.services s on s.id=a.service_id and s.tenant_id=t where a.tenant_id=t and (a.start_at at time zone 'Europe/Rome')::date=p_date and (role_name<>'operator' or a.operator_id=own_operator)),'[]'::jsonb));end $function$;
...
begin if auth.uid() is null or coalesce((auth.jwt()->>'is_anonymous')::boolean,false) then raise exception 'Accesso non autorizzato';end if;
...
if length(trim(coalesce(p_name,''))) not between 1 and 120 or length(coalesce(p_surname,''))>120 or (normalized_email is not null and normalized_email !~ '^[^[:space:]@]+@[^[:space:]@]+[.][^[:space:]@]+$') or length(coalesce(normalized_phone,''))>25 or (normalized_email is null and normalized_phone is null) then raise exception 'Dati non validi';end if;
insert into public.customers(tenant_id,nome,cognome,email,telefono) values(p_tenant,trim(p_name),nullif(trim(coalesce(p_surname,'')),''),normalized_email,normalized_phone) returning id into result;return result;end $function$;
...
revoke all on function public.prenow_owner_dashboard(text,date) from public,anon;
grant execute on function public.prenow_owner_dashboard(text,date) to authenticated;
