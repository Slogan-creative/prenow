...
export default async function ManualBooking({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<Record<string,string|undefined>>}){
 ...
 async function book(form:FormData){
  ...
  const back=new URLSearchParams({service:clean(form.get('service')),operator:clean(form.get('operator')),date:clean(form.get('date'),10),customer});
 ...
 const preserved=<><input type="hidden" name="service" value={q.service||''}/><input type="hidden" name="operator" value={q.operator||''}/><input type="hidden" name="date" value={q.date||''}/>{selectedCustomer&&<input type="hidden" name="customer" value={selectedCustomer.id}/>}</>;
 const tabParams=(customerMode:string)=>new URLSearchParams({service:q.service||'',operator:q.operator||'',date:q.date||'',customer_mode:customerMode,...(selectedCustomer?{customer:selectedCustomer.id}:{})});
 return <><Link href={`/titolare/${encodeURIComponent(slug)}/clienti`}>← Clienti</Link><h1>{tenant.data.nome}: prenotazione manuale</h1><p>{selectedCustomer?`Nuovo appuntamento per ${selectedCustomer.nome} ${selectedCustomer.cognome||''}. Scegli servizio, operatore, data e orario.`:'Scegli un cliente esistente oppure creane uno nuovo. Orari, assegnazioni e disponibilità vengono sempre verificati.'}</p>
 ...
 <form method="get" className="settings-card">{selectedCustomer&&<input type="hidden" name="customer" value={selectedCustomer.id}/>}<label>Servizio <select required name="service" defaultValue={q.service}><option value="">Seleziona</option>{services.data?.map(s=><option key={s.id} value={s.id}>{s.nome}</option>)}</select></label><label>Operatore <select required name="operator" defaultValue={q.operator}><option value="">Seleziona</option>{operators.data?.map(o=><option key={o.id} value={o.id}>{o.nome}</option>)}</select></label><label>Data <input required type="date" name="date" defaultValue={q.date}/></label><button className="btn btn-primary">Cerca orari</button></form>
